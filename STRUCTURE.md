...
| `src/configs/`         | `*.config.ts`     | `env.config.ts`         | `env.config.ts` — the one parser of `process.env`, plus the `APP_ENV` derivation helpers — `env-consistency.config.ts` (`assertEnvConsistent`, the boot checks), `rate-limit-store.config.ts`, and `mailer.config.ts` (lazy, memoised nodemailer transporter) |
