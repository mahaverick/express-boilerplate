...
| `src/configs/`         | `*.config.ts`     | `env.config.ts`         | `env.config.ts` — the only file allowed to read `process.env` — `rate-limit-store.config.ts`, and `mailer.config.ts` (lazy, memoised nodemailer transporter)                                                                             |
...
| `src/observability/`       | `tracing.ts` — the OpenTelemetry SDK bootstrap, loaded via `--import` before the app (see `package.json`'s `dev`/`start` scripts). A single fixed-name entrypoint, not a suffix-conventioned module type — it and `console.*`/`process.env` are exempted the same way `env.config.ts`/`logger.service.ts`/`index.ts` are. See [CLAUDE.md](CLAUDE.md)'s "Observability" section.                       |
