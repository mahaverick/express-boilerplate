...
| `src/repositories/`    | `*.repository.ts` | `user.repository.ts`    | `base.repository.ts` (abstract; shared soft-delete/update-tracking policy), `user.repository.ts`, `user-token.repository.ts`, `email-log.repository.ts` (append-only; does NOT extend `base.repository.ts` — see its own header comment)                                                                                        |
| `src/services/`        | `*.service.ts`    | `database.service.ts`   | `database.service.ts`, `redis.service.ts`, `mailer.service.ts` (`sendMail` — never rejects, see its own header comment)                                                                                                                                                                                                         |
...
| `src/middlewares/`     | `*.middleware.ts` | `error.middleware.ts`   | `error.middleware.ts`, `request-id.middleware.ts`, `auth.middleware.ts` (`requireAuth`), `rate-limit.middleware.ts`, `content-type.middleware.ts` (`requireJsonContentType`, a CSRF control — see [SECURITY.md](SECURITY.md))                                                                                                   |
...
| `src/configs/`         | `*.config.ts`     | `env.config.ts`         | `env.config.ts` — the one parser of `process.env`, plus the `APP_ENV` derivation helpers — `env-consistency.config.ts` (`assertEnvConsistent`, the boot checks), `database.config.ts` (`databaseClientOptions`, the pool options), `rate-limit-store.config.ts`, and `mailer.config.ts` (lazy, memoised nodemailer transporter) |
