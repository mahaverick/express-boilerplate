...
// `'verify_email'` is deliberately excluded — see
// notification-preference.repository.ts's `NON_DISABLEABLE_EMAIL_TYPES` for
// the full reasoning (a user who disabled it would lock themselves out of
// ever verifying their own account). That set is module-private to the
// repository and stays that way: it governs what `isChannelEnabled`
// resolves at READ time regardless of what any row says, which is a
// stronger, unconditional guarantee than this list. This is the WRITE-side
// mirror of the same rule — narrower in principle (today it happens to
// match exactly) but independently necessary, because without it a client
// could still successfully `PUT` a `{ notificationType: 'verify_email',
// emailEnabled: false }` row; the repository would silently keep honouring
// email delivery regardless, but the settings UI this endpoint serves would
// show the user a toggle that lies about its own effect.
//
// NOTIFICATION_TYPES has exactly one entry today, so this filter currently
// produces an EMPTY array — there is nothing left to configure until a
// second notification type with a disableable channel ships. That is not a
// bug to special-case away: `preferenceEntrySchema` below rejects every
// `notificationType` with the same clear, per-field message whether the
// configurable list has one entry or none, so `PUT /preferences` already
// answers 400 correctly — see the "PUT /api/v1/notifications/preferences"
// describe block in tests/integration/api/notification.test.ts, which
// exercises exactly this (a `verify_email` update, an empty array, and an
// unknown type all landing on the same clear-message 400) — without this
// module needing a separate branch for "zero configurable types" versus
// "some, but not this one".
const NON_DISABLEABLE_NOTIFICATION_TYPES: ReadonlySet<string> = new Set(['verify_email'])
