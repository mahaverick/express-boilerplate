...
//
// A rate limiter is built by `createRateLimiter(...)` (rate-limit.middleware.ts)
// the moment `createAuthRouter()` assembles the auth routes — which happens
// before the startup sequence has connected anything. Nothing in this codebase connects
